...
class SIFT:
    ...
    def assign_orientations(self, keypoints, image):
        ...
        for key_point in keypoints:
            ...
            radius = int(np.ceil(1.5 * sigma)) # size of the patch is proportional to the keypoint's scale (sigma), which This ensures that the patch covers the region of interest around the keypoint.

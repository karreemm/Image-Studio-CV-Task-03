    ...
    def compute_descriptors(self, keypoints, image):
        ...
        for kp in keypoints:
            ...
            if norm > 0:
                ...
                descriptor = np.clip(descriptor, 0, 0.2) # Any value in the descriptor greater than 0.2 is set to 0.2 to reduces the influence of large gradient magnitudes, which could dominate the descriptor and make it less robust to noise or small variations.
                norm = np.linalg.norm(descriptor)
                if norm > 0:
                    descriptor = descriptor / norm

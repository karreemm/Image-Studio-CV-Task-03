...
class SIFT:
    ...
    def assign_orientations(self, keypoints, image):
        ...
        for key_point in keypoints:
            ...
            hist_smoothed = np.convolve(hist, [1, 1, 1], mode='same') / 3 # Smoothing reduces noise and ensures that the dominant orientation is more robustly identified. The smoothing is done using a simple 3-bin moving average:
Each bin is replaced by the average of itself and its two neighbors.

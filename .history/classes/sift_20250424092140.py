...
class SIFT:
    ...
    def assign_orientations(self, keypoints, image):
        ...
        for key_point in keypoints:
            ...
            sigma = key_point.size / 2 #  diameter of the keypoint in the image. This value is proportional to the scale (or sigma) at which the keypoint was detected during the SIFT algorithm.

            radius = int(np.ceil(1.5 * sigma)) # size of the patch is proportional to the keypoint's scale (sigma), which ensures that the patch covers the region of interest around the keypoint.

...
class SIFT:
    ...
    def detect_extrema(self):
        ...
        for octave_index in range(self.num_octaves):
            ...
            for level in range(1, self.num_levels - 2): # starts from 1 and ends at self.num_levels - 2 because we compare each pixel in a DoG image with its neighbors in the previous and next DoG levels, so there must be previous and next levels!
